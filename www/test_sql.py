...
__author__ = 'Allan Ni'
# 接着 models.py 继续， 对数据库进行测试， 数据库的知识参考廖老师的教程：
...
grant select, insert, update, delete on allan.* to 'www-data'@'localhost';
...
async def test(loop):                      # *** 注意此处的密码填自己设的密码 ***
    await orm.create_pool(loop=loop, user='root', password='password', db='allan')
                                           # *** 注意此处的密码填自己设的密码 ***
